...
rev 需要懒更新(翻转)
*/
ll revtag(ll u, ll rev){
    segtree[u].sum = (segtree[u].rt - segtree[u].lt + 1) - segtree[u].sum; //u区间子节点数量-当前数量
    segtree[u].rev ^= 1; //更新懒标记
    return 1;
}
...
ll pushdown(ll u){
    if(segtree[u].rev){
        revtag(u<<1, segtree[u].rev);
        revtag(u<<1|1, segtree[u].rev);
        segtree[u].rev = 0;
    }
    return 1;
}
...
int main() {
    ...
    while(m--){
        cin >> op;
        cin >> x >> y; 
        if(op==0) rangeupdate(1, x, y, 1);
        else cout << rangequery(1, x, y) << endl;
    }
    return 0;
}